...
latinobarometro2020 %>% 
  group_by(iso3) %>%
  summarise(
  promedio = weighted.mean(dim1_01, wt = wt, na.rm = T)) %>%
...
latinobarometro %>% 
  ...
  labs(
    ...
    subtitle = "Países seleccionados. 1998-2020.",
    caption = "Fuente: elaboración propia en base a Latinobarómetro"
  ) + 
  theme(
    axis.title.x = element_blank(),
  ...
  scale_x_continuous(breaks = c(1998, 2002, 2007, 2009, 2010, 2020)) +
  facet_wrap(~iso3, ncol = 4)
...
latinobarometro2020 %>%
  ...
  scale_x_continuous(limits = c(.3, .75), breaks = seq(.3, .75, .05))
...
latinobarometro2020 %>%
  ...
  tbl_svysummary(include = c(edad, sexo2, dim1_01, clase_subjetiva4, just_dist_ingresos_dic, pro_mercado_dic, ayuda_pobres_dinero_f, ayuda_gobierno_tramo_f, pago_impuestos_dic, aceptabilidad_desigualdad, corrupcion_f, ideologia, arreglo_impuestos),
                 label = c(edad ~ "Edad", 
                           sexo2 ~ "Sexo", 
                           dim1_01 ~ "Índice de estratificación", 
